...
import letterVert from 'assets/lettersVert.js'
import letterFrag from 'assets/lettersFrag.js'
...
export default class AFrameHome extends React.Component{
    constructor(){
        super();
    ...
    componentWillUnmount(){
        delete AFRAME.components['handle-events']
    }
    render(){
        let offset2 = -0.05;
        let offset1 = -0.1;
        ...
        return (
            <div onKeyDown={()=>this.onKeyPress()} onClick={()=>this.onKeyPress()}>
                <a-scene animation-timeline="timeline: #flyin">
                    ...
                    {
                        letterSet1.map((element, key)=>{
                            return <a-gltf-model id={`ele_${key}_1`} key={key} src={`#${element}`} scale="0.05 0.05 0.05" rotation="0 -90 0" position={`${offset1+key*0.013} 2.24 -0.1`}
                                animation__entry={`property: position; from:${offset1+key*0.013} 4 -1; to:${offset1+key*0.013} 2.24 -0.1; autoplay:false; dur:500; loop: false; elasticity: 1000; easing: easeOutElastic;`}
                            ></a-gltf-model>
                        })
                    }
                    ...
                    {/* scale="0.05 0.05 0.05" rotation="0 -90 0" position={`${offset2+key*0.013} 2.21 -0.1 */}
                    {
                        ...
                        letterSet2.map((element, key)=>{
                            return <a-gltf-model material="shader: skyGradient; colorTop: #353449; colorBottom: #BC483E; side: back" id={`ele_${key}_2`} key={key} src={`#${element}`} scale="0.05 0.05 0.05" rotation="0 -90 0" position={`${offset2+key*0.013} 2.21 -0.1`}
                            animation__entry={`property: position; from:${offset1+key*0.013} 4 -1; to:${offset1+key*0.013} 2.21 -0.1; autoplay:false; dur:500; loop: false; elasticity: 1000; easing: easeOutElastic;`}
                            ></a-gltf-model>
                        })
                    }
                    </a-entity>
            ...
            <Key>Click anywhere to Enter</Key>
            </div>
        )
    }
}